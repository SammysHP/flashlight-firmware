...
#define FIRMWARE_VERS ((1<<5) | 0)	// v1.0 (in bits: mmm.nnnnn, mm=major @, nnnnn=minor#)
		...
		else
		{
			...
			if (wIdleTicks > DBL_CLICK_TICKS)	// Too much delay between clicks - time out the fast clicks
			{
				...
				switch (modeState)
				{
				 ...
				 case RAMPING_ST:
				 case TACTICAL_ST:
					if (fastClicks == 2)						// --> double click: direct to MAX/turbo
					{
					...
					else if (fastClicks == 4)				// --> 4 clicks: set toggel Ramping/Tactical Mode
					{
