...
uint8_t off_state(Event event, uint16_t arg) {
    ...
    if (event == EV_enter_state) {
        ...
        indicator_led_update(indicator_led_mode & 0x07, 0);
    ...
    // go back to sleep eventually if we got bumped but didn't leave "off" state
        ...
        if (arg > HOLD_TIMEOUT) {
            go_to_standby = 1;
            #ifdef USE_INDICATOR_LED
            indicator_led_update(indicator_led_mode & 0x07, arg);
            #elif defined(USE_AUX_RGB_LEDS)
    ...
    // 7 clicks: change indicator LED mode
        ...
        uint8_t mode = (indicator_led_mode & 0x07) + 1;
        ...
        if (mode == 1) { mode ++; }
        #endif
        indicator_led_mode = (indicator_led_mode & 0b11111000) | mode;
